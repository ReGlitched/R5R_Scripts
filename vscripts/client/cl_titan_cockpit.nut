...
void function TitanCockpitInit( entity cockpit )
{
	...
	thread TitanCockpitAnimThink( GetLocalViewPlayer().GetCockpit(), cockpit.e.body )
...
void function DisplayFrontierRank( bool isFirstBoot = true )
{
	...
	string titanClass = "vanguard"//titanLoadout.titanClass
...
function TitanCockpit_EMPFadeScale( entity cockpit, elapsedMod = 0 )
{
	...
	//ToDo: Fade in/out from last frames amount so it doesnt pop
	//ToDo: Make strength var to control max fade ( less strength returns max of like 0.5 )
...
function TonemappingUpdateAfterOpeningCockpit() //Deprecated //TODO: Delete this
{
	local duration = 3
	local tonemapMin = 2
	local tonemapMax = 5
