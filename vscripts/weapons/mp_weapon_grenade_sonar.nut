...
global float SONAR_GRENADE_RADIUS 			= 1250.0
global float SONAR_GRENADE_PULSE_DURATION 	= 4.5
...
void function SonarGrenadeThink( entity projectile )
{
	...
	if ( IsValid( owner ) && owner.IsPlayer() )
	{
		...
		foreach ( weapon in offhandWeapons )
		{
			...
			{
				float duration = weapon.GetWeaponSettingFloat( eWeaponVar.grenade_ignition_time ) + 0.75
	...
	while ( IsValid( projectile ) && Time() < endTime )
	{
		...
		foreach ( player in players )
			Remote_CallFunction_Replay( player, "ServerCallback_SonarPulseFromPosition", pulseOrigin, SONAR_GRENADE_RADIUS, 1.0, false )
...
void function OnSonarTriggerEnter( entity trigger, entity ent )
{
	...
	if ( IsHostileSonarTarget( trigger.GetOwner(), ent ) && ent.GetTeam() != TEAM_TICK )
