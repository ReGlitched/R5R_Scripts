	...
	foreach ( player in GetPlayerArrayOfTeam( victim.GetTeam() ) )
	{
		...
		if ( GetGameState() <= eGameState.Playing &&
				...
				Bleedout_IsBleedingOut( player ) &&
						!Bleedout_AnyOtherSquadmatesAliveAndNotBleedingOut( player ) &&
	...
	foreach ( p in GetPlayerArrayOfTeam( team ) )
	{
		if ( IsAlive( p ) &&
				Bleedout_IsBleedingOut( p ) &&
						!Bleedout_AnyOtherSquadmatesAliveAndNotBleedingOut( p ) &&
...
{
	int stepIndex = GetGlobalNetInt( "characterSelectLockstepIndex" )
